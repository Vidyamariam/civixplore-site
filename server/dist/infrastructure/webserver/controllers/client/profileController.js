...
export const updateProfileController = async (req, res) => {
    try {
        ...
        let imageUrl = null;
        if (req.file) {
            // File was uploaded successfully, use the file path
            imageUrl = `/uploads/customers/profiles${req.file.filename}`;
        }
        const updatedData = {
            fullName,
            phoneNumber,
...
export const getProfileController = async (req, res) => {
    try {
        ...
        const email = req.query.email; // Fetch email from the route parameter
        ...
        const customerProfile = await customerRepository.getProfile(email);
