...
export class MongoAdminRepository {
    ...
    async approveUser(id) {
        try {
            const user = await UserModel.findByIdAndUpdate(id, { status: 'approved', isApproved: true }, { new: true } // Return the updated document
            );
            if (!user) {
    ...
    async blockUser(id, isBlocked) {
        try {
            ...
            const result = await UserModel.updateOne({ _id: userId }, // Find the user by ID
    ...
    async updateSubscriptionPlan(id, updatedPlan) {
        try {
            ...
            const flattenedUpdatedPlan = "plan" in updatedPlan && updatedPlan.plan
                ? updatedPlan.plan
                : updatedPlan;
            ...
            const result = await SubscriptionPlanModel.findByIdAndUpdate(id, updateQuery, { new: true });
